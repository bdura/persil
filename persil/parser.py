...
class Parser(Generic[Output]):
    ...
    def __init__(
        self,
        wrapped_fn: Callable[[Sequence, int], Result[Output]],
    ):
        """
    ...
    def __call__(self, stream: Sequence, index: int) -> Result[Output]:
        return self.wrapped_fn(stream, index)

    def then(self, other: "Parser[T]") -> "Parser[T]":
        """
        ...
        @Parser
        def bound_parser(stream: Sequence, index: int) -> Result[T]:
    ...
    def skip(self, other: "Parser") -> "Parser[Output]":
        """
        Returns a parser which, if the initial parser succeeds, will
        ...
        @Parser
        def bound_parser(stream: Sequence, index: int) -> Result[Output]:
            result = self(stream, index)
    ...
    # <<
    def __lshift__(self, other: "Parser") -> "Parser[Output]":
        return self.skip(other)

    def parse(self, stream: Sequence) -> Output:
        """Parses a string or list of tokens and returns the result or raise a ParseError."""
        (result, _) = (self << eof).parse_partial(stream)
        return result

    def parse_partial(self, stream: Sequence) -> tuple[Output, Sequence]:
        """
    ...
    def bind(self, bind_fn: Callable[[Output], "Parser[T]"]) -> "Parser[T]":
        @Parser
        def bound_parser(stream: Sequence, index: int):
            result = self(stream, index)
    ...
    def result(self, value: T) -> "Parser[T]":
        ...
        @Parser
        def result_parser(stream: Sequence, index: int) -> Result[T]:
            res = self(stream, index)
    ...
    def times(self, min: int, max: int | None = None) -> "Parser[list[Output]]":
        """
        ...
        @Parser
        def times_parser(stream: Sequence, index: int) -> Result[list[Output]]:
            values = []
            times = 0
            result = None
    ...
    def many(self) -> "Parser[list[Output]]":
        """
    ...
    def at_most(self, n: int) -> "Parser[list[Output]]":
        """
    ...
    def at_least(self, n: int) -> "Parser[list[Output]]":
        """
    ...
    def optional(self, default: T | None = None) -> "Parser[Output | T | None]":
        """
        ...
        @Parser
        def optional_parser(stream: Sequence, index: int) -> Result[Output | T | None]:
            res = self(stream, index)
    ...
    def until(
        ...
        @Parser
        def until_parser(stream: Sequence, index: int) -> Result[list[Output]]:
            values: list[Output] = []
            times = 0
    ...
    def sep_by(
        ...
        """
        zero_times: Parser[list[Output]] = success([])
    ...
    def desc(self, description: str) -> "Parser[Output]":
        """
        ...
        @Parser
        def desc_parser(stream: Sequence, index: int) -> Result[Output]:
            result = self(stream, index)
            if isinstance(result, Ok):
                return result
    ...
    def should_fail(self, description: str) -> "Parser[Result[Output]]":
        """
        ...
        @Parser
        def fail_parser(stream: Sequence, index: int):
            res = self(stream, index)
            if isinstance(res, Ok):
                return Err(index, [description], stream)
            return Ok(res, index)
    ...
    def __add__(self, other: "Parser[Output]") -> "Parser[Output]":
        @Parser
        def inner(stream: Sequence, index: int) -> Result[Output]:
    ...
    def __or__(self, other: "Parser[T]") -> "Parser[Output | T]":
        @Parser
        def inner(stream: Sequence, index: int) -> Result[Output | T]:
            res1 = self(stream, index)
...
def generate(gen: Callable[[], Generator[Parser, Any, T]]) -> Parser[T]:
    """
    ...
    @Parser
    @wraps(gen)
    def generated(stream: Sequence, index: int) -> Result[T]:
